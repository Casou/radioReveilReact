...
function cacheMp3Files() {
    DEFAULT_MP3_FILES = [];
    console.log("Caching MP3 files");
    fs.readdir(DEFAULT_MP3_FOLDER_ABSOLUTE, (err, files) => {
        files.forEach(file => {
            let filePath = DEFAULT_MP3_FOLDER + "/" + file;
...
function encodeSongPath(path) {
    return removeDiacritics(path.replaceAll(" ", "_"));
}
...
const server = http.createServer(function(request, response) {
    ...
    if (extname === ".json") {
        ...
        response.end(handleAjax(filePath), 'utf-8');
    } else {
...
function handleAjax(requestUrl) {
    switch (requestUrl) {
        case './listDefaultMp3Files.json':
            return listMp3Files();
        default :
            console.log("URL json " + requestUrl + " unknown");
    }
}
